...
class BirthdayTest(TestCase):
    ...
    def test_ordering(self):
        ...
        doys = [
            getattr(
                obj, 'birthday_dayofyear_internal'
            ) for obj in TestModel.objects.order_by_birthday(True)
        ]
        self.assertEqual(doys, [365, 2, 1])

        years = [
            obj.birthday.year for obj in TestModel.objects.order_by('birthday')
        ]
    ...
    def test_manager(self):
        ...
        self.assertEqual(
            TestModel.objects.get_birthdays(jan1).count(), 1
        )
        self.assertEqual(
            TestModel.objects.get_upcoming_birthdays(30, jan1).count(), 2
        )
        self.assertEqual(
            TestModel.objects.get_upcoming_birthdays(30, jan1, False).count(),
            1
        )

        dec31 = date(year=2010, month=12, day=31)
        self.assertEqual(
            TestModel.objects.get_birthdays(dec31).count(), 1
        )
        self.assertEqual(
            TestModel.objects.get_upcoming_birthdays(30, dec31).count(), 3
        )
        ...
        self.assertEqual(
            TestModel.objects.get_upcoming_birthdays(30, dec31, False).count(),
            2
        )
        self.assertTrue(
            TestModel.objects.get_birthdays().count() in [0, 1]
        )
    ...
    def test_exception(self):
        ...
        self.assertRaises(
            FieldError, BirthdayField().contribute_to_class, BrokenModel,
            'another_birthday'
        )
